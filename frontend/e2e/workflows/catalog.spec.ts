...
test('catalog creation, stock correction, filtering and audit history persist through the API', async ({ page }) => {
  ...
  await page.getByPlaceholder('e.g. PG-TIDE-002').fill(sku);
  await page.getByPlaceholder('e.g. 12.99').fill('12.50');
  await page.getByPlaceholder('e.g. Tide Pods Clean Breeze 38ct').fill('Regression stock item');
  ...
  await selectWithOption(page, 'Select Warehouse').selectOption({ label: 'Main Distribution Center' });
...
test('duplicate catalog SKUs are rejected without creating a second product', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'New Product' }).click();
  await page.getByPlaceholder('e.g. PG-TIDE-002').fill('SKU-ELEC-001');
  await page.getByPlaceholder('e.g. 12.99').fill('20.00');
  await page.getByPlaceholder('e.g. Tide Pods Clean Breeze 38ct').fill('Duplicate product attempt');
  ...
  await expect(page.getByText('Failed to create product. Ensure the SKU is unique.')).toBeVisible();
