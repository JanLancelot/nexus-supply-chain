...
import com.nexus.supplychain.service.AnalyticsService;
...
class AnalyticsAndNotificationTests {
    ...
    void setUp() throws InterruptedException {
        ...
        adminUser = userRepository.findByEmail("admin@example.test").orElse(null);
        staffUser = userRepository.findByEmail("staff@example.test").orElse(null);
    ...
    void testNotificationEndpoints() {
        ...
        // Get notifications
        com.nexus.supplychain.dto.NotificationListResponse listResponse = notificationService.getNotificationsForCurrentUser();
    ...
    @Test
    void retentionDeletesOnlyReadNotificationsPastTheConfiguredAge() {
        Notification unread = notificationRepository.save(Notification.builder().user(adminUser).type("RETENTION")
                .message("unread evidence").createdAt(OffsetDateTime.now().minusDays(45)).isRead(false).build());
        Notification read = notificationRepository.save(Notification.builder().user(adminUser).type("RETENTION")
                .message("already read").createdAt(OffsetDateTime.now().minusDays(45)).isRead(true).build());
        Notification recent = notificationRepository.save(Notification.builder().user(adminUser).type("RETENTION")
                .message("recent read").createdAt(OffsetDateTime.now().minusDays(1)).isRead(true).build());
        notificationService.pruneOldNotifications();
        assertTrue(notificationRepository.existsById(unread.getId()));
        assertFalse(notificationRepository.existsById(read.getId()));
        assertTrue(notificationRepository.existsById(recent.getId()));
    }

    @Test
    void replayedNotificationEventIsIdempotentAcrossTransactions() {
        UUID eventId = UUID.randomUUID();
        Notification first = notificationService.createNotificationForEvent(adminUser, "EVENT", "transition", eventId);
        Notification repeated = notificationService.createNotificationForEvent(adminUser, "EVENT", "transition", eventId);
        assertEquals(first.getId(), repeated.getId());
        assertEquals(1, notificationRepository.findAll().stream()
                .filter(notification -> eventId.equals(notification.getSourceEventId())).count());
    }
}
