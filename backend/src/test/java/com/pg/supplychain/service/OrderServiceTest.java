...
class OrderServiceTest {
    ...
    void testUpdateOrderStatus_RejectsDeliveryOverflow() {
        UUID orderId = UUID.randomUUID();
        Product product = Product.builder().id(UUID.randomUUID()).stockQuantity(Integer.MAX_VALUE).build();
        Order order = Order.builder().id(orderId).status(OrderStatus.SHIPPED)
