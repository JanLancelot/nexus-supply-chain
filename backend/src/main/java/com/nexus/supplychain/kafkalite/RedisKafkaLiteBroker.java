...
public class RedisKafkaLiteBroker implements KafkaLiteBroker, SmartInitializingSingleton {
    ...
    public String healthStatus() {
        return useFallback ? "MEMORY_FALLBACK" : "REDIS_FALLBACK";
    }
                ...
                if (message != null) {
                    ...
                    if (handlers != null) {
                        for (Consumer<String> handler : handlers) {
                            deliverWithRetry(topic, handler, message);
                        }
                    }
                }
    ...
    private void deliverWithRetry(String topic, Consumer<String> handler, String message) throws InterruptedException {
        for (int attempt = 0; attempt < 4 && running; attempt++) {
            try {
                ...
                if (attempt < 3) {
                    ...
                    // Development fallback has process-lifetime durability when Redis is unavailable.
                    // Keep failed payloads for inspection instead of treating failed handlers as successful.
                    if (!useFallback) {
                        try {
                            redisTemplate.opsForList().rightPush("kafka_topic_" + topic + ".DLT", message);
                            return;
                        } catch (RuntimeException deadLetterFailure) {
                            log.error("KafkaLite: Redis dead-letter publication failed; retaining the payload locally", deadLetterFailure);
                        }
                    }
                    inMemoryQueues.computeIfAbsent(topic + ".DLT", key -> new LinkedBlockingQueue<>()).offer(message);
                    log.error("KafkaLite: Handler exhausted retries for topic {}. Payload retained in {}.DLT", topic, topic, failure);
                }
            }
        }
    }
