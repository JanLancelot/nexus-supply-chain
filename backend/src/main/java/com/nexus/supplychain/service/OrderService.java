...
import com.nexus.supplychain.dto.*;
...
public class OrderService {
    ...
    public OrderResponse updateOrderStatus(UUID orderId, OrderStatusUpdateRequest request) {
        ...
        if (targetStatus == OrderStatus.DELIVERED) {
            ...
            for (OrderItem item : sortedItems) {
                ...
                validateProductWarehouse(product, order.getWarehouse());
                ...
                newProductState.put("orderId", order.getId());
                newProductState.put("orderNumber", order.getOrderNumber());
                newProductState.put("quantityReceived", item.getQuantity());

                // Audit the product stock increment
                auditService.logChange(
                        "Product",
                        product.getId(),
                        "ACTION_ORDER_RECEIPT",
                        oldProductState,
                        newProductState
                );
            }
    ...
    private void validateProductWarehouse(Product product, Warehouse warehouse) {
        if (warehouse == null || product.getWarehouse() == null
                || !warehouse.getId().equals(product.getWarehouse().getId())) {
            throw new BadRequestException("Product " + product.getSku()
                    + " must belong to the order destination warehouse");
        }
    }
