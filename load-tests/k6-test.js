...
export function setup() {
  ...
  console.log(`[Setup] Starting load test initialization targeting: ${baseUrl}`);
...
export default function (data) {
  ...
  if (isStaff) {
    ...
    if (Math.random() < 0.1 && categories.length && warehouses.length) {
      ...
      const uniqueSku = `SKU-LOAD-${__VU}-${__ITER}-${Date.now()}`;
    ...
    if (Math.random() < 0.2 && products.length) {
      // Prioritize the newly created product if available, else pick a random one
